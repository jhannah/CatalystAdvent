...
C<$c-E<gt>res-E<gt>headers-E<gt>header()> is the main method we'll be adding 
...
You can see we are using the B<date> plugin in the content template, which 
pulls in the C<modtime> from that template and not the B<footer.tt> or 
B<header.tt>, then we set up our stash to use in our B<Root.pm> Controller.
...
Please make sure you understand what headers do before implementing, and what 
your site needs. If in doubt, use a search engine or speak to the Catalyst
community. Thanks!
