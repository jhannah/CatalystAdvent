=head1 Day X - HTML::Widget tricks

Custom markup with HTML::Widget

=head2 Introduction

Today we'll show you how to generate custom markup from your widget using a
custom container, and how to add custom elements to your forms.

Let's add a simple feedback form to our Cheer project. First thing, add the 
L<Catalyst::Plugin::HTML::Widget> in the plugin list, in Cheer.pm. It brings 
some advantages, you can read all about it in the POD. Then open your Root.pm
and type your form:
...
The generated markup is as follows (I added the whitespace for easy reading): 
...
This may not be exactly what you seek , and if you don't want to build the 
form manually in the template, you'll have to build a custom container.

Let's just make a container that does nothing, but will allow us to continue
further. Put this in lib/Cheer/Container.pm . The code is just copied 
from HTML::Widget::Container, ready for our customisations:
...
That's it! You can check the new output imediatly. For more funky edits,
...
Also keep in mind that doing this in the Controller is a B<bad ideea>, but
with HTML::Widget it's pretty difficult to avoid.
