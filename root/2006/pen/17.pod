...
Any non-trivial project needs solid QA, and most Catalyst projects are not 
trivial. This article will guide through some specific way to test your 
controllers.

General Perl testing is beyond the scope of this discussion, but you can read 
about it in Test::Tutorial, Test::Simple, Test::More, and chromatic's great 
book, Testing Perl.
...
For example, T::W::M::C tests will answers questions like: Can I click this 
link? What does this page contain? If I submit this form, will the content be 
updated? It is also nice for testing complex user scenarios. 
...
Because we don't use an external http server, we can use any kind of magick to 
alter our app's environment. A good way to test if the number of days is ok
is faking Today() in Cheer::Model::Now, using magick of course. Let's just 
replace the tests like this:
...
Note we could have done something cleaner using Time::Warp or Test::MockTime if
Date::Calc was written in pure Perl.
...
Oops, apparently our app is not ready for these exceptional cases. Let's fix 
it and pass the tests. Here's the diff for our template:
...
While testing your app from outside is very useful to check if your features 
are implemented as planned, more rigurous testing is needed to ensure the
B<corectness of your code>.
...
You have there a fake model that always has 5 days till xmas, and a fake
context that returns our fake model. 

Later on, after the action is ran, you can pick up the results from the fake context's stash.

When unit testing controllers, just make sure you provide all the needed 
objects in the action's argument list. Make sure every method your code calls 
has predictable results. And then, just check the stash and you're done.
