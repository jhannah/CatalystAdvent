...
=item * replace all messages with function calls to a translation module,
...
preffered) in order to allow non-technical people to localize your software.

=head2 Setting the URLs , using chained actions

For this exercise, let's just make our app available to both English and 
Romanian viewers. 
...
Ok, that's it . Let's internationalize the messages now.

=head2 I18N with $c->loc

Just replace your messages with calls to $c->loc. Replace your
interpolated variabiles with [_1] , [_2] etc. Look:
...
Everything should still be working, but now we are ready to 
localize it.

=head2 L10N with a little help from xgettext.pl

xgettext.pl comes with the Locale::Maketext::Lexicon distribution.
You just pass it a list of files and it collects all localized strings
...
Of course, in a real project you have tons of files. Here's what I do:
 ...
 (find root/ -name '*.tt' ; find lib/MyApp/Controller -name '*.pm' ) | 
  xargs /path/to/xgettext.pl -o lib/MyApp/I18N/ro.po
...
you the localized version. Craciun fericit :)
