...
to need to put things in the stash. This isn't so much of a problem, but
...
C<load_group> is called first, and then C<members> is called next.)
 ...
 sub load_group : Chained('/')
   ...
   my $group = ... # load group from Model here
...
It enables us do the following:
 ...
 sub load_group :Chained('/')
   ...
   my $group :Stashed = ... # load group from Model here
 }
...
It has to be said however that some people view this module as a bad
thing due to the modules it uses L<PadWalker>, L<Array::RefElem>, 
L<Devel::Caller>, L<Devel::LexAlias> and L<attributes>. However I've not
