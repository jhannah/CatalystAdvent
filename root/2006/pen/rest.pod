...
I<transferring> it; and thus possibly changing it's I<state>.
...
The final piece of the REST puzzle are Content Types.  These are used to
...
representation we receive of a given resource is the one we are best suited 
...
L<Catalyst::Action::REST> helps you deal with the different verbs by extending
...
(L<Catalyst::Action::Serialize>) and to deserialize data received from them
...
Creating the proper HTTP responses to clients makes every-ones lives easier.
...
I love monkeys as much as the next guy, but even I balk at having it in my
