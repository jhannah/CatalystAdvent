...
at the end of this article.  The proceeding paragraph owes quite a bit
...
Retrieve, Update, Delete) acronym common in databases.

=item B<Content Types>

The final piece of the REST puzzle are Content Types.  These are used
to describe the format of a resource; how the various parts of our
machine should process it.  In practice, we specify the Content Type
to ensure that the representation we receive of a given resource is
the one we are best suited to process. (C<text/xml> for XML,
C<text/x-yaml> for YAML, C<text/x-json> for JSON, etc.)
...
Truthfully, Catalyst already helps you here.  It's incredibly flexible
...
all the implemented methods.  (In this case, it would containing GET
...
L<Catalyst::Action::Serialize> and L<Catalyst::Acton::Deserialize>.
...
received from them (L<Catalyst::Action::Deserialize>).  11 different
...
Creating the proper HTTP responses to clients makes every-ones lives
...
here, or download the tarball from
L<http://dev.catalyst.perl.org/foo>.

Our example application will be a simple User database that tracks
User ID's, Full Names, and Descriptions of users.

So lets start be determining what our Nouns will be, and what Verbs
are valid for them:
...
A reference implementation will appear on the Catalyst subversion
respository in due course.  This article will be updated with the url
...
files exist in the "data" directory) Lets start with some yaml.
