=head2 Entry 16 FastCGI, second visit

Here's a simple, newbie perspective on FastCGI deployment.
...
live deployed using FastCGI on Apache.

Because deployment is dependent on many things external to Catalyst,
we're particularly interested in feedback on deployment.  If you
experience issues, the #catalyst irc channel on irc.perl.org or the
Catalyst mailing list (see the end of this article for details) should
be your first port of call.

FastCGI is a recommended deployment route because it offers the
similar speed to mod_perl, but allows you to restart your app without
restarting the whole Apache server (FastCGI doesn't just limit you to
Apache either).  FastCGI also allows you to run multiple versions of
the same application with the same web server.
...
You can use the external server with sockets (a file which facilitates
inter-process communication), or with a tcp connection.  Tcp offers the
most flexibility in the end (the application can easily run on a
different machine to the server), so this is the way we are going.
...
httpd.conf:
 ...
 # or Alias /myapp/ /path/to/myapp/script/myapp_fastcgi.pl/ to run as non root
...
L<Catalsyt Mailing List|http://lists.rawmode.org/mailman/listinfo/catalyst>.

For further information, please see the L<Catalyst::Engine::FastCGI>
and <Catalyst::Cookbook> documentation.
...
Copyright 2006 Vlad Dan Dascalescu.  This document can be freely
