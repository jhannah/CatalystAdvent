...
to use this in a situation where you have a preexisting model but are
...
C<ChainedEg::ControllerBase::LoadModel> which provides the C<list>
...
populate $cE<gt>stash.  Looking at the subroutine declaration for the
...
dispatcher should look for more more subroutines to run before
...
the C<resultset> action is called, and does not use capture any
arguments into C<$c-E<gt>request-E<gt>path> (this will make more sense
in the next section).

On the other hand, the list declares has C<Args> as an attribute.
This means that it's an end point, and after the subroutine is
executed, the appropriate C<end> action is called.  Note that the
expected number of arguments (in this case none) are put into
C<$c-E<gt>request-E<gt>path> by the Catalyst dispatcher.

The result of this code is that when we call our application from it's
base path with the additional path C</en/foo/> we can easily provide
localisation via the language subroutine in C<Controller::Root> and
when nothing is in the path after the name of the controller, we
exectute the C<list> method.
...
The code responsible for this is as follows.  In the L<ControllerBase>
package, which is a base class for L<Controller::Foo> (C<use base
'MyAdvApp::ControllerBase::LoadModel>) provides the following method:
...
It takes 1 argument for internal use by the controller (placed into
the variable $item_id in the first line of the sub) and uses this to
populate the stash
 
COME TO THINK OF IT, I DON'T GET WHY THERE'S THE 'BASE' PART ARG TO
THE CHAINED ATTRIB
...
which as we can see by the use of C<Args> rather than C<CaptureArgs>
is the end point.

The edit action is somewhat similar.  By this point you can that there
is a pattern developing. The edit_foo endpoint is called from
C<Controller::Foo> after the load_model subroutine from the base
...
As we mentioned in the beginning of this discussion, the end action
...
out the application presented here into a working example.  You would
put the C<MyDatabase> directory in your application's C<lib> directory
(or anywhere else in C<@INC>).  Assuming that you put the SQLite database
C<example.db> in the application root you could create the Catalyst
model via calling a helper in the following way:
...
Words:  Kieren Diment C<E<lt>diment@gmail.comE<gt>>
Code:   Robert 'phaylon' Sedlacek C<E<lt>rs@474.atE<gt>>
