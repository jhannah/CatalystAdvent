...
function is quite small:
    ...
    sub geocode { my ( $self, $location ) = @_; return
        $self->{'.geocoder'}->geocode( location => $location ); }
...
root action we defined earlier:
    ...
    sub root : Chained('base') : PathPart('') : Args(0) { my ( $self,
        $c ) = @_; if ( $c->req->params->{location} ) {
        $c->stash->{location} = $c->model('Geocode') ->geocode(
        $c->req->params->{location}); } $c->forward('View::JSON'); }
