...
and it's dependences installed, do so now with:
...
Now point your web browser to L<http://127.0.0.1:3000> (substitute if a 
different IP address if your development server is running on a 
...
browser (you might need to Shift-Reload).  You should see startup and 
...
formatting for Log4perl (we will see how to modify those messages in the 
next section).
...
appender to send the log output to a file.  See documentation on 
...
messages (C<PatternLayout> can be used with all appenders).  Here is
a quick list of the formatting options supported by C<PatternLayout> (see 
...
lines similar to the following (to avoid word wrapping, the time/date 
fields have been shortened to "[...]"):
...
One extremely useful features of Log4perl is that it can email you when 
things go really wrong with your application (or other events of 
interest occur).  First, let's install the 
...
Then, open the C<Log4perl.conf> file and edit it to match:
    ...
    # The the from and to email addresses here
    ...
    # Although the rootLoger has a level of WARN, we can
    # further filter this appender with the Threshold option
    # because we only want emails to be sent for very severe issues
...
FAQ:
C<http://log4perl.sourceforge.net/releases/Log-Log4perl/docs/html/Log/Log4perl/FAQ.html>
