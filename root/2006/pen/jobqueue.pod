...
first 6 elements will determine how often your job will be run (it's 
L<http://www.adminschoice.com/docs/crontab.htm#Crontab%20file|crontab syntax>
). The next one is unused for now, so you can ignore it. The last
one represents the request that the job will send to your application. It's an
URL. Your job looks like this:
...
=head2 Draw the conclusions

The engine is not yet production quality and not recommended for running
critical jobs. But as you could see, you can run any action from your
controllers and have the results mailed to you and/or logged.

=head3 AUTHOR

Grün Christian-Rolf (kixx) <kiki@bsdro.org>
