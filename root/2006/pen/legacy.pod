...
I was asked to update a rather old website which has traditionally been
maintained with Dreamweaver, and uses nested tables to provide the
layout.  I needed to draw from a variety of sources - my own and outside
databases, and data gathered from screen scrapers, RSS feeds, and so on.
So my models were a mess, my employer was completely inflexible about
the tools used to do the job, and in any case the same tools are due to
be replaced with a new (expensive) content management system in a couple
of months' time.
...
=item In C<MyApp::Controller::Root-E<gt>auto>
...
=item Run C<wget> to make a static mirror
