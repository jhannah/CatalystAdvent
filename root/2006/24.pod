...
applications, the Cheer application from day 1 of this years advent
...
namespace on L<CPAN>.  There are also a few fewer modules in the
L<Catalyst::Controller> name space, not to forget the
...
techniques you can use to extend this.  However remember, while
scaffolding can provide you with an attractive looking kick-start to a
project, depending on its complexity, you working around the
...
However, scaffolding does have its uses, just less often than people
