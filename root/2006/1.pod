...
what accounts for L<its power|Catalyst::Manual::About>.
...
repository.  Instructions for obtaining it are given at the end off
this article.
...
now, and add a template.  Here's the patch which includes changes to
...
that you go through the tutorials from L<Catalyst::Manual::Tutorial>
