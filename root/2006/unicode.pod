...
their language to add (poor) support for multiple langauge (or more
...
but it also holds true for a string like C<???>.  What you think of as
characters, Perl thinks of as characters.

However, the problem arises when you confuse a I<byte> (or I<octet>)
with a I<character>.  Although 'a' is both a character and an octet, a
UTF-8 character can be made up of more than one octet.  This is where
the problem begins because the Internet (and terminal emulators,
filesystems, etc.) have no concept of a character -- they only
transmit, receive, or store I<octets>.  

=head1 Using Unicode Correctly

By default, Perl thinks of everything as an octet, and hence won't
...
made up of characters, you have to explicitlty tell Perl that the
...
from the terminal or web broweser into perl -- will also appear to
work).  
...
etc.) where the invalid data is coming from, and add a prople
C<decode> or C<encode> statement.
...
C<decode> all incoming data into Perl characters, and C<encode>
outgoing characters output to unicode (UTF-8) octets.  (Only the body
is encoded -- you should avoid using non-ASCII characters in the
headers.)

This will work in most cases, but if you're using a legacy character
encoding, you will have to do the conversion manually with the
L<Encode> module (and explicitly specify what charset to use).
...
This us important to be careful about, because mixing characters and
...
UTF-8, your site will probably work (but don't do this!).  If you
decode some things, and then mix in un-decoded data, perl will
interpert each octet of the un-decoded data as a full character.  When
