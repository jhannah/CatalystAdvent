...
In December 2005 Andy Grudman wrote a very short streaming server for
...
throttles the deliver of large media blobs on a per user basis.  This
server is really a proof of concept which shows you the basic
technique to get started with that task.

Because it's relatively old (by Catalyst standards) there is no
C< Controller::Root > - the root controller is in C< Streaming.pm >.

Next thing you need to do is to copy an mp3 file into the application
root and call it C< test.mp3 >).

Next up, you're going to have to find an mp3 player that supports
streaming - we just send the file directly so we'd have to mess around
with embedding and Javascript to get it to play in the browser
properly.  So I  used mplayer:
