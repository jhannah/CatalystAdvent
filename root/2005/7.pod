...
the worker MPM.  In Debian, the following commands should get you going.
...
Static files can be served directly by Apache for a performance boost.  In
a two-tiered setup, they should be served by the frontend server.
...
This will let all files within root/static be handled directly by Apache. 
