=head1 Catalyst Advent - Day 15 - RSS feed.
...
normal view action first to get the objects, then handle the output differntly
...
Advent Calendar. Then the code would look something like this:
    ...
    sub rss : Local {
        ...
        while( my $entry=$c->stash->{entries}->next ) {
            ...
            $entry->link( $c->uri_for($entry->link) );
            $entry->issued( DateTime->from_epoch(epoch   => $entry->created) );
...
A little more code in the controller, but with this approach your pretty sure
